...
from data import ImageFilelist, ImageFolder, CelebA
import torch
...
def get_all_data_loaders(conf):
    ...
    selected_attrs = conf["gen"]['selected_attrs']

    train_loader_u = get_loader_unlabeled(os.path.join(conf['data_root'], 'unlabeled'), batch_size, True,
                                          new_size, height, width)
    train_loader_l = get_loader_labeled(os.path.join(conf['data_root'], 'labeled'), conf['label_root'],
                                        selected_attrs, batch_size, True, new_size, height, width)
...
def get_loader_unlabeled(input_folder, batch_size, train, new_size=None,
                         height=256, width=256, crop=True):
    transform_list = [transforms.ToTensor(),
                      transforms.Normalize((0.5, 0.5, 0.5),
                                           (0.5, 0.5, 0.5))]
